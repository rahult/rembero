...
normalization as recall schema ranking. Each reason/token pair scores once. Results sort
by score descending, then fact, rule, constraint, then canonical clause text. There is no
learning, frequency feedback, personalization weight, or hidden relevance state.
...
Durable source text is already credential-redacted by the store. Ranking reads at most
4,096 source characters across a clause's sources and marks `rankingSourceTruncated` when
more exists; returned provenance remains complete. Source text never leaves the process.

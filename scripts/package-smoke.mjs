  ...
  run(
    ...
    [
      ...
      "import { IncompleteHistoryError, IntegrityViolationError, MemoryStore, OperationConflictError, analyzeKnowledgeTopology, applyMemoryProposal, applyRuleChangeProposal, assertTentativeFacts, auditKnowledgeRules, browseKnowledgeGraph, canonicalizeKnowledge, checkIntegrity, connectKnowledgeGraph, createKnowledgeBundle, diffRecordedKnowledge, evaluate, evaluateQuerySpec, explainKnowledge, explainWhyNot, inspectConflicts, isAggregateRule, materialize, parseProgram, parseQuery, parseQuerySpec, planKnowledgeRepair, profileKnowledge, proposeRememberText, recallQuestion, resolveTentativeFacts, retrieveQuestion, reviewTentativeClaims, runKnowledgeChecks, searchKnowledge, selectExplanationGraph, selectRecallSchema, serializeKnowledgeBundle, simulateKnowledge, sqliteDatalogExecutionMode, verifyKnowledgeBundle } from 'rembero'; " +
        ...
        "const negative = await recallQuestion({ store: negativeStore, llm: negativeLlm }, 'Where does Zoe work?'); " +
        "if (negative.status !== 'no_match' || negativeLlm.calls !== 2 || negative.answer !== negative.whyNot?.summary || !negative.answer.includes('Required fact works_at(zoe, Company) is missing')) throw new Error('public grounded negative recall failed'); " +
        ...
        "if (memoryApplied.audit.topology.factCount !== 1 || memoryProposalStore.load('default')[0]?.head.args[1]?.value !== 'initech') throw new Error('public reviewed memory application API failed'); " +
